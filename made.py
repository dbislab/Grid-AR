...
class MADE(nn.Module):
    ...
    def update_masks(self, invoke_order=None):
        ...
        # if the input is encoded the mask for the input layer should be changed
        if self.input_encoding is not None:
...
if __name__ == '__main__':
    ...
    configs_with_input_bins = [
        ...
        (4, [10, 10, 10], 2 + 3 + 2 + 2, [2, 3, 2, 2], False),
        # (2, [10], 2 + 5, [2, 5], True),
    ...
    for nin, hiddens, nout, input_bins, direct_io in configs_with_input_bins:
        ...
        for k in range(nout):
            ...
            # depends = (one_hot.grad[0].numpy() != 0).astype(np.uint8)
